(function() {
  ...
  describe('miniSlider', function() {
    ...
    describe('navigation', function() {
      beforeEach(function() {
        return this.plugin = new $.miniSlider(this.$element, {
          transitionSpeed: 0
        });
      });
      it('should add the current class to the current slide', function() {
        expect(this.plugin.slider.slides[0].element).toHaveClass('current');
        expect(this.$element.children('.current').length).toBe(1);
        this.plugin.slider.next();
        expect(this.plugin.slider.slides[1].element).toHaveClass('current');
        expect(this.plugin.slider.container.find('.current').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[0].element).toHaveClass('current');
        expect(this.plugin.slider.container.find('.current').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[3].element).toHaveClass('current');
        return expect(this.plugin.slider.container.find('.current').length).toBe(1);
      });
      it('should add the previous class to the previous slide', function() {
        expect(this.plugin.slider.slides[3].element).toHaveClass('previous');
        expect(this.$element.children('.previous').length).toBe(1);
        this.plugin.slider.next();
        expect(this.plugin.slider.slides[0].element).toHaveClass('previous');
        expect(this.plugin.slider.container.find('.previous').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[3].element).toHaveClass('previous');
        expect(this.plugin.slider.container.find('.previous').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[2].element).toHaveClass('previous');
        return expect(this.plugin.slider.container.find('.previous').length).toBe(1);
      });
      it('should add the next class to the next slide', function() {
        expect(this.plugin.slider.slides[1].element).toHaveClass('next');
        expect(this.$element.children('.next').length).toBe(1);
        this.plugin.slider.next();
        expect(this.plugin.slider.slides[2].element).toHaveClass('next');
        expect(this.plugin.slider.container.find('.next').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[1].element).toHaveClass('next');
        expect(this.plugin.slider.container.find('.next').length).toBe(1);
        this.plugin.slider.previous();
        expect(this.plugin.slider.slides[0].element).toHaveClass('next');
        return expect(this.plugin.slider.container.find('.next').length).toBe(1);
      });
      it('should navigate via the next and previous links', function() {
        this.plugin.slider.nextLink().trigger('click');
        expect(this.plugin.slider.currentIndex).toBe(1);
        this.plugin.slider.previousLink().trigger('click');
        expect(this.plugin.slider.currentIndex).toBe(0);
        this.plugin.slider.previousLink().trigger('click');
        expect(this.plugin.slider.currentIndex).toBe(3);
        this.plugin.slider.nextLink().trigger('click');
        return expect(this.plugin.slider.currentIndex).toBe(0);
      });
      return describe('pagination', function() {
        beforeEach(function() {
          return this.paginationLinks = this.plugin.slider.$pagination.children();
        });
        it('should allow users to navigate', function() {
          $(this.paginationLinks[3]).find('a').trigger('click');
          expect(this.plugin.slider.currentIndex).toBe(3);
          $(this.paginationLinks[1]).find('a').trigger('click');
          expect(this.plugin.slider.currentIndex).toBe(1);
          $(this.paginationLinks[0]).find('a').trigger('click');
          return expect(this.plugin.slider.currentIndex).toBe(0);
        });
        return it('should update the current link class', function() {
          expect($(this.paginationLinks[3]).find('a').trigger('click').parent()).toHaveClass('current-pagination');
          expect($('.pagination').find('.current-pagination').length).toBe(1);
          expect($(this.paginationLinks[1]).find('a').trigger('click').parent()).toHaveClass('current-pagination');
          expect($('.pagination').find('.current-pagination').length).toBe(1);
          expect($(this.paginationLinks[0]).find('a').trigger('click').parent()).toHaveClass('current-pagination');
          return expect($('.pagination').find('.current-pagination').length).toBe(1);
        });
      });
    });
    return describe('callbacks', function() {
      ...
      return it('should call onComplete callback when the animation is complete', function() {
        var anotherCallback, plugin;
        anotherCallback = sinon.spy();
        plugin = new $.miniSlider(this.$element, {
          onTransition: anotherCallback,
        ...
        expect(this.callback.called).toBeFalsy();
        plugin.slider.next();
        expect(this.callback.calledAfter(anotherCallback)).toBeTruthy();
