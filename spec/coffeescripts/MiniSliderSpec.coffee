describe 'miniSlider', ->
  ...
  describe 'navigation', ->
    beforeEach ->
      @plugin = new $.miniSlider( @$element, { transitionSpeed: 0 } )

    it 'should add the current class to the current slide', ->
      expect( @plugin.slider.slides[0].element ).toHaveClass( 'current' )
      expect( @$element.children( '.current' ).length ).toBe 1

      @plugin.slider.next()
      expect( @plugin.slider.slides[1].element ).toHaveClass( 'current' )
      expect( @plugin.slider.container.find( '.current' ).length ).toBe 1

      @plugin.slider.previous()
      expect( @plugin.slider.slides[0].element ).toHaveClass( 'current' )
      expect( @plugin.slider.container.find( '.current' ).length ).toBe 1

      @plugin.slider.previous()
      expect( @plugin.slider.slides[3].element ).toHaveClass( 'current' )
      expect( @plugin.slider.container.find( '.current' ).length ).toBe 1

    it 'should add the previous class to the previous slide', ->
      expect( @plugin.slider.slides[3].element ).toHaveClass( 'previous' )
      expect( @$element.children( '.previous' ).length ).toBe 1

      @plugin.slider.next()
      expect( @plugin.slider.slides[0].element ).toHaveClass( 'previous' )
      expect( @plugin.slider.container.find( '.previous' ).length ).toBe 1
      ...
      @plugin.slider.previous()
      expect( @plugin.slider.slides[2].element ).toHaveClass( 'previous' )
      expect( @plugin.slider.container.find( '.previous' ).length ).toBe 1

    it 'should add the next class to the next slide', ->
      expect( @plugin.slider.slides[1].element ).toHaveClass( 'next' )
      expect( @$element.children( '.next' ).length ).toBe 1

      @plugin.slider.next()
      expect( @plugin.slider.slides[2].element ).toHaveClass( 'next' )
      expect( @plugin.slider.container.find( '.next' ).length ).toBe 1
      ...
      @plugin.slider.previous()
      expect( @plugin.slider.slides[0].element ).toHaveClass( 'next' )
      expect( @plugin.slider.container.find( '.next' ).length ).toBe 1


    it 'should navigate via the next and previous links', ->
      @plugin.slider.nextLink().trigger( 'click' )
      expect( @plugin.slider.currentIndex ).toBe 1

      @plugin.slider.previousLink().trigger( 'click' )
      expect( @plugin.slider.currentIndex ).toBe 0

      @plugin.slider.previousLink().trigger( 'click' )
      expect( @plugin.slider.currentIndex ).toBe 3
    ...
    describe 'pagination', ->
      beforeEach ->
        @paginationLinks = @plugin.slider.$pagination.children()

      it 'should allow users to navigate', ->
        $( @paginationLinks[3] ).find('a').trigger( 'click' )
        expect( @plugin.slider.currentIndex ).toBe 3

        $( @paginationLinks[1] ).find('a').trigger( 'click' )
        expect( @plugin.slider.currentIndex ).toBe 1

        $( @paginationLinks[0] ).find('a').trigger( 'click' )
        expect( @plugin.slider.currentIndex ).toBe 0

      it 'should update the current link class', ->
        expect( $( @paginationLinks[3] ).find('a').trigger( 'click' ).parent() ).toHaveClass 'current-pagination'
        expect( $('.pagination').find( '.current-pagination' ).length ).toBe 1

        expect( $( @paginationLinks[1] ).find('a').trigger( 'click' ).parent() ).toHaveClass 'current-pagination'
        expect( $('.pagination').find( '.current-pagination' ).length ).toBe 1

        expect( $( @paginationLinks[0] ).find('a').trigger( 'click' ).parent() ).toHaveClass 'current-pagination'
        expect( $('.pagination').find( '.current-pagination' ).length ).toBe 1
  ...
  describe 'callbacks', ->
    ...
    it 'should call onComplete callback when the animation is complete', ->
      anotherCallback = sinon.spy()
      plugin          = new $.miniSlider( @$element, { onTransition: anotherCallback, onComplete: @callback, autoPlay: false, transitionSpeed: 0 } )

      expect( @callback.called ).toBeFalsy()
      plugin.slider.next()
      expect( @callback.calledAfter( anotherCallback ) ).toBeTruthy()
